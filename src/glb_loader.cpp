...
bool LoadGLB(MyMesh &m, tinygltf::Model &outModel, const std::string &filename) {
    ...
    for (const auto &mesh : outModel.meshes) {
        ...
        for (const auto &primitive : mesh.primitives) {
            ...
            vcg::tri::Allocator<MyMesh>::AddVertices(m, posAcc.count);
            for (size_t i = 0; i < posAcc.count; ++i) {
                m.vert[vertOffset + i].P() =
            ...
            if (primitive.indices >= 0) {
                ...
                vcg::tri::Allocator<MyMesh>::AddFaces(m, indAcc.count / 3);
                ...
                for (size_t i = 0; i < indAcc.count; i += 3) {
                    ...
                    if (texCoords) {
                        m.face[faceIdx].WT(0) = MyFace::TexCoordType(
                            texCoords[idx[0] * texStride], texCoords[idx[0] * texStride + 1]);
                        m.face[faceIdx].WT(1) = MyFace::TexCoordType(
                            texCoords[idx[1] * texStride], texCoords[idx[1] * texStride + 1]);
                        m.face[faceIdx].WT(2) = MyFace::TexCoordType(
...
bool SaveGLB(MyMesh &m, const tinygltf::Model &originalModel, const std::string &filename) {
    ...
    int accPos = addAccessor(bvPos, rawPos.size() / 3, TINYGLTF_COMPONENT_TYPE_FLOAT,
                             TINYGLTF_TYPE_VEC3, &posMin, &posMax);
    int accNor = addAccessor(bvNor, rawNor.size() / 3, TINYGLTF_COMPONENT_TYPE_FLOAT,
                             TINYGLTF_TYPE_VEC3, nullptr, nullptr);
    int accUV  = addAccessor(bvUV, rawUV.size() / 2, TINYGLTF_COMPONENT_TYPE_FLOAT,
                             TINYGLTF_TYPE_VEC2, nullptr, nullptr);
    int accInd = addAccessor(bvInd, rawIndices.size(), TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT,
                             TINYGLTF_TYPE_SCALAR, nullptr, nullptr);
