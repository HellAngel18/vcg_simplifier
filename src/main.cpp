...
// 使用带纹理支持的简化类
class MyCollapse : public vcg::tri::TriEdgeCollapseQuadricTex<
                        MyMesh, 
                        MyVertexPair, 
                        MyCollapse, 
                        vcg::tri::QuadricTexHelper<MyMesh> > { // 注意这里的 Helper
public:
...
void LogStatus(MyMesh &m, const char* stage) {
    printf("[%s] V:%d F:%d\n", stage, m.VN(), m.FN());
}
...
bool CheckTexCoords(MyMesh &m) {
    if (!tri::HasPerWedgeTexCoord(m)) return false;
    // 简单检查是否有 NaN
    for(auto &f : m.face) {
        if(f.IsD()) continue;
        for(int i=0; i<3; ++i) {
            if (std::isnan(f.WT(i).u()) || std::isnan(f.WT(i).v())) return false;
        }
    }
    return true;
}
...
int main(int argc, char **argv) {
    if (argc < 2) {
        printf("Usage: mesh-simplifier -i <input.obj> [-r <ratio 0.0-1.0>] [-o <output.obj>]\n");
        return 1;
    }
    ...
    if (lastSlash != string::npos) chdir(pathStr.substr(0, lastSlash).c_str());
    ...
    if(err) {
        ...
        if(tri::io::Importer<MyMesh>::ErrorCritical(err)) return -1;
    }
